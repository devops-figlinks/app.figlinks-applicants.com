...
const useQuestionsHook = ({
  ...
  useEffect(() => {
    ...
    if (questionNo === 0 && !fixedScreenshotFlags.current.start) {
      ...
      prevEyeLeftCount.current = detectionCounts.current.eye_left_count || 0;
      prevEyeRightCount.current = detectionCounts.current.eye_right_count || 0;
      prevMultiFaceCount.current = detectionCounts.current.multiple_face_detected_count || 0;
      prevCameraDisableCount.current = counts.video || 0;
    ...
    try {
      ...
      const payload = {
        ...
        proctoring_activity_count: {
          ...
          total_face_verification_mismatch_time: calculateTotalEyeTime(eyeTimeIntervals.faceVerification),
        },
