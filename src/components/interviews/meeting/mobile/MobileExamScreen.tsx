...
const MobileMcqExamScreen: FC<IMcqExamBlock & IQuestioningBlock> = (props) => {
    ...
    // Add a ref to track if we've handled the Safari intro issue
    const safariIntroHandled = useRef(false);
    ...
    useEffect(() => {
        if (isSafari && currentStage === 'intro' && isInterviewStarted && !safariIntroHandled.current) {
            safariIntroHandled.current = true;
  ...
  <div className="flex flex-col h-screen bg-[#2d2d2d] text-white p-5 box-border capitalize rounded-none">
        ...
        <div className="flex items-center text-[#fcfcfc] text-base font-medium capitalize gap-1.25">
          {remainingTime > 0 && (
            <p className="bg-white px-3 py-1 rounded-sm flex items-center gap-2">
              <Image src="/mcq-mobile/timer-icon.svg" width={20} height={20} alt="" />
    ...
    {currentStage !== "conclusion" && (
      ...
      >
        ...
        <Image src="/mcq-mobile/arrow-right.svg" alt="" width={25} height={25} />
        </div>
      </Button>
    )}
    <Loading
